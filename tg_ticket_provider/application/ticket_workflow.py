...
from tg_ticket_provider.application.keyboards import (
...
from tg_ticket_provider.domain.entities import (
    Priority,
    Ticket,
    TicketStatus,
    transition_done,
...
from tg_ticket_provider.domain.message_format import render_ticket_caption
class TicketWorkflow:
    ...
    async def accept_in_group(
        self, session: AsyncSession, ticket_id: int, user_id: int
    ) -> Ticket:
        ...
        dm_main, _ = await self._messages.send_dm_ticket(
            ...
            reply_markup_json=dm_take_or_return_kb(ticket_id),
        )
    ...
    async def return_ticket(
        ...
        if ticket.group_message_id:
            await self._messages.edit_ticket_message(
                ...
                thread_id=ticket.department_thread_id,
                text_html=text,
                reply_markup_json=group_accept_kb(ticket_id),
            )
        if dm_mid:
            await self._messages.edit_ticket_message(
                chat_id=user_id,
    ...
    async def mark_in_progress(
        ...
        if ticket.dm_message_id:
            await self._messages.edit_ticket_message(
                ...
                thread_id=None,
                text_html=text_d,
                reply_markup_json=dm_work_kb(ticket_id),
            )
    ...
    async def attach_photo(
        self, session: AsyncSession, ticket_id: int, user_id: int, file_id: str
    ) -> None:
