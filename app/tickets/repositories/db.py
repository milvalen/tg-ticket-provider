...
class TicketRepository:
    async def try_assign_if_open(
        ...
        *,
        ...
        row = await session.get(Tickets, ticket_id)
        if (
            row is None
    ...
    async def get_by_id(self, session: AsyncSession, ticket_id: UUID) -> Ticket | None:
        row = await session.get(Tickets, ticket_id)
        if row is None:
            return None
        return ticket_from_row(row)
    ...
    async def list_in_progress_for_user(
        ...
        q = (
            select(Tickets)
            .where(Tickets.assignee_user_id == user_id)
            .where(Tickets.status == TicketStatus.IN_PROGRESS.value)
        )
    ...
    async def add_attachment(
        ...
        row = TicketAttachments(ticket_id=ticket_id, file_id=file_id, added_by_user_id=user_id)
        session.add(row)
        await session.flush()
